      ...
      <div id="cuerpo" class="container_12">
          <div class="ui-widget grid_12 ui-widget-header" id="encabezado" style="background: url('<?php echo image_path($sf_user->getAttribute('fondoCabColegio', 'test/fondoCab')) ?>') repeat-x">
            <div class="centrar">
                ...
                <span class="titulo">Elecciones <?php echo $sf_user->getAttribute('abreviaturaEntidadEducativa','Gobierno Escolar').' '.$sf_user->getAttribute('añoEleccionesEntidadEducativa',date('Y'))?></span>
            </div>
            <?php echo link_to('Votación', '@'.($sf_user->hasAttribute('abreviaturaEntidadEducativa')?'homepageEntidadEducativa?abreviatura='.$sf_user->getAttribute('abreviaturaEntidadEducativa'):'homepage'),array('id' => 'go_to_admin'));?>
        </div>
